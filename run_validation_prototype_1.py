...
if __name__ == "__main__":
    ...
    parser.add_argument('--saving_name', type = str, default = "validation_prototype_1", \
    ...
    parser.add_argument('--finetune_path_encoder', type = str, \
    ...
    default= 'D:\\Thesis_Repo\\GhostModel\\thesis_final_deepfillv3\\models\\Model_version_prototype_1__Enc_without__L1_L2_epoch220_batchsize1.pth', \
    #".\models\Model_encBot_L1_L2_epoch420_batchsize1.pth", \
        help = 'load name path of models')
    parser.add_argument('--finetune_path_decoder', type = str,  \
    # default = 'D:\\Thesis_Repo\\GhostModel\\new_net\\models\\Model_version_SECOND_TESTTTT_RUN_TWO_dec_new_2_L1_L2_epoch220_batchsize1.pth', \
    default= 'D:\\Thesis_Repo\\GhostModel\\thesis_final_deepfillv3\\models\\Model_version_prototype_1__Dec_without__L1_L2_epoch220_batchsize1.pth', \
    # ".\models\Model_dec_L1_L2_epoch420_batchsize1.pth", \
        help = 'load name path of models')
    parser.add_argument('--finetune_path_autoencoder', type = str, \
    # default = 'D:\\Thesis_Repo\\GhostModel\\new_net\\models\\Model_version_SECOND_TESTTTT_RUN_TWO_autoen2_new_2_L1_L2_epoch220_batchsize1.pth', \
    default= 'D:\\Thesis_Repo\\GhostModel\\thesis_final_deepfillv3\\models\\Model_version_prototype_1__AE_with__L1_L2_epoch220_batchsize1.pth', \
    # ".\models\Model_autoen2_L1_L2_epoch420_batchsize1.pth", \
        help = 'load name path of models')
    parser.add_argument('--finetune_path_bottleneck', type = str,  \
    default= 'D:\\Thesis_Repo\\GhostModel\\thesis_final_deepfillv3\\models\\Model_version_prototype_1__bottleneck__L1_L2_epoch220_batchsize1.pth', \
    ...
    parser.add_argument('--sample_path', type = str, default = './validation', \
    ...
    logg('#-'*40,verbose=False)
    logg('Namespace variables', verbose=False)
    # for item in  vars(opt).items():
    #     logg(f'\t--{item[0]}\t {item[1]}', verbose=False)
    # logg(opt)
    ...
    # network_name = opt.load_name.split('\\')[-1].split('.')[0]
    # log_path = f"{opt.log_path}/validation_model_{opt.saving_name}_{opt.model}.log"
    # logg(log_path)
    # logging.basicConfig(filename=log_path, level=logging.DEBUG)
    logg(f'This is the validation file of model {opt.model} and the networks {opt.finetune_path_encoder}, \
         \n {opt.finetune_path_decoder}, \
           \n {opt.finetune_path_autoencoder}, \
              \n {opt.finetune_path_bottleneck}')
    logg(f'used data is {opt.baseroot}')
    logg(f"{opt}")


    Validation_Prototype_1(opt)
