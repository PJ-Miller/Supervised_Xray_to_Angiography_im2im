...
def Prototype_4Y_opt_pix2pix(opt):
    ...
    #     save_model(encoder_bottle_net, (epoch + 1), opt, 'enc_without2without_')
    #     save_model(decoder_net, (epoch + 1), opt, 'dec_without2without_')
        
    #     # save_model(ae_net, (epoch + 1), opt, 'autoen1')
    #     save_model(ae_net2, (epoch + 1), opt, 'base_ae_with2with_')
    ...
    for epoch in range(opt.epochs): 
        ...
        # Save the model
        save_model(bottleneck_net, (epoch + 1), opt, '_bottleneck_without2with')
        save_model(discriminator384, (epoch + 1), opt, 'discriminator_proto4Y_')
        save_model(perceptualnet, (epoch + 1), opt, 'perceptual_proto4Y_')
    ...
    print("Perc loss", Per_loss_values)
    print("GAN loss", GAN_loss_values)
    print("Rec loss", Rec_loss_values)
    print("Overall loss", Overall_loss_values)
    ...
    if opt.validation == 1:
        ...
        for batch_idx, (grayscale_without, grayscale_with, mask) in enumerate(dataloader):
            ...
            utils_sample.sample_general_bottleneck(opt, grayscale_with, out_ae, grayscale_without, out_decoder, opt.sample_path, (1 + 1),saving_name=str(f'proto_4Y_validation_{batch_idx}'))
