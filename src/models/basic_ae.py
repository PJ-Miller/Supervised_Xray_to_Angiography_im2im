...
# , torchvision.transforms.ToTensor()
    ...
    for epoch in range(opt.epochs):
        ...
        for batch_idx, (grayscale_without, grayscale_with) in enumerate(dataloader): 
            ...
            if opt.model ==0 :
                out_wholeimg_with = grayscale_with * (1 - mask) + out_with * mask   # grayscale_with * (1 - mask) + mask
        ...
        if epoch % opt.checkpoint_image== opt.checkpoint_image-1:
            utils_sample.sample_general(opt, grayscale_without, out_with, grayscale_with, grayscale_without, opt.sample_path, (epoch + 1),saving_name=opt.saving_name+'_new')
    ...
    if opt.validation == 1:
        ...
        # Define the dataloader
        # dataloader = DataLoader(trainset, batch_size = opt.batch_size, shuffle = True, num_workers = opt.num_workers, pin_memory = True)
        dataloader = DataLoader(trainset, batch_size = 1, shuffle = False, num_workers = opt.num_workers, pin_memory = True)
        ...
        for batch_idx, (grayscale_without, grayscale_with, mask) in enumerate(dataloader):
            ...
            utils_sample.sample_one(opt, grayscale_without, opt.sample_path, (1 + 1),saving_name=str(f'grayscale_without_{opt.saving_name}_{batch_idx}'))
            
            utils_sample.sample_one(opt, out_ae, opt.sample_path, (1 + 1),saving_name=str(f'out_ae_{opt.saving_name}_{batch_idx}'))
            utils_sample.sample_one(opt, grayscale_with, opt.sample_path, (1 + 1),saving_name=str(f'grayscale_with_{opt.saving_name}_{batch_idx}'))


            utils_sample.sample_general_bottleneck(opt, grayscale_without, out_ae, grayscale_with, grayscale_without, opt.sample_path, (1 + 1),saving_name=str(f'validation_{batch_idx}'))
...
# , torchvision.transforms.ToTensor()
    ...
    for epoch in range(opt.epochs):
        ...
        for batch_idx, (grayscale_without, grayscale_with) in enumerate(dataloader): 
            ...
            out_with_1 = ae_net.encoder(grayscale_without)  
            out_bot = bottleneck_net(out_with_1)
            out_with = ae_net.decoder(out_bot)
        ...
        save_model(ae_net, (epoch + 1), opt, 'autoencder_idea3_')
        save_model(bottleneck_net, (epoch + 1), opt, 'bot_net_idea3_')


        if epoch % opt.checkpoint_image== opt.checkpoint_image-1:
            utils_sample.sample_general(opt, grayscale_without, out_with, out_with, grayscale_without, opt.sample_path, (epoch + 1),saving_name=opt.saving_name+'_new')
    ...
    if opt.validation == 1:
        ...
        for batch_idx, (grayscale_without, grayscale_with, mask) in enumerate(dataloader):
            ...
            # out_bottle =  bottleneck_net(out_enc_net1) 
            # out_decoder = ae_net2.decoder(out_bottle)   
            # out_ae1 = ae_net(grayscale_without)                      # grayscale_with

            out_with_1 = ae_net.encoder(grayscale_without)  
            out_bot = bottleneck_net(out_with_1)
            out_ae = ae_net.decoder(out_bot)
