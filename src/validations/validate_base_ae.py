...
#         for batch_idx, (grayscale_without, _, _) in enumerate(dataloader):

#             # grayscale_with = grayscale_with.cuda()
#             grayscale_without = grayscale_without.cuda()
#             # mask = mask.cuda()

#             out_ae = ae_net(grayscale_without)   
...
#             utils_sample.sample_general_bottleneck(opt, grayscale_without, out_ae, grayscale_without, out_ae, opt.sample_path, (1 + 1),saving_name=str(f'valid_AE_base_{batch_idx}'))
...
--finetune_path_autoencoder D:\thesis_pix2pix\models\Model_version_MALAKA_generator_basic_ae__L1_L2_epoch70_batchsize1.pth
...
def Validate_base_ae(opt):
    ...
    if opt.in_channels == 1:
        ...
        transforms = torchvision.transforms.Compose([
                torchvision.transforms.Resize((opt.imgsize,opt.imgsize)) #,
                # torchvision.transforms.Normalize((0.5,), (0.1,))
                # torchvision.transforms.CenterCrop(opt.imgsize)
                                ])
    ...
    if opt.validation == 1:
        ...
        for batch_idx, (grayscale_without, grayscale_without2,_) in enumerate(dataloader):
            # grayscale_with = grayscale_with.cuda()
            # print(torch.Tensor(grayscale_without).dtype)
            # print(torch.Tensor(grayscale_without2).dtype)
            grayscale_without = grayscale_without.cuda()
            grayscale_without2 = grayscale_without2.cuda()
            # mask = mask.cuda()
            # torchvision.utils.save_image(grayscale_without,"takis.png")
            out_ae = ae_net(grayscale_without)   
            out_ae2 = ae_net(grayscale_without2)   
            # # inference
            # out_enc_net1 = encoder_bottle_net(grayscale_without)  # grayscale_without
            # out_bottle =  bottleneck_net(out_enc_net1) 
            # out_decoder = ae_net2.decoder(out_bottle)   
            ...
            # utils_sample.sample_general_bottleneck(opt, grayscale_without, grayscale_without, grayscale_without2, grayscale_without2, opt.sample_path, (1 + 1),saving_name=str(f'TESSSSS2_{batch_idx}'))
            utils_sample.sample_one(opt, grayscale_without, opt.sample_path, (1 + 1),saving_name=str(f'output1_{opt.saving_name}_{batch_idx}'))
            
            utils_sample.sample_one(opt, grayscale_without2, opt.sample_path, (1 + 1),saving_name=str(f'output2_{opt.saving_name}_{batch_idx}'))
            utils_sample.sample_one(opt, out_ae, opt.sample_path, (1 + 1),saving_name=str(f'input_{opt.saving_name}_{batch_idx}'))
            utils_sample.sample_one(opt, out_ae2, opt.sample_path, (1 + 1),saving_name=str(f'input2_{opt.saving_name}_{batch_idx}'))
            
            utils_sample.sample_general_bottleneck(opt, grayscale_without, out_ae, grayscale_without2, out_ae2, opt.sample_path, (1 + 1),saving_name=str(f'{opt.saving_name}_{batch_idx}'))
